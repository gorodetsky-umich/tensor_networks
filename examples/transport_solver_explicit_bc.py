...
def source_term(indices, tt, disc, draw=False):
    ...
    funcs = [None] * 3
    funcs[0] = np.ones((disc[0].shape[0])) 
    funcs[0][0] = 0
    # funcs[0][-1] = 0
    funcs[1] = np.ones((disc[1].shape[0]))
    funcs[2] = np.ones((disc[2].shape[0]))
    mask = tt_rank1('mask', indices, funcs)
...
class Problem:
    ...
    def __init__(self, indices, c=1.0, kscale=5e-1, dtscale = 0.8):
        ...
        self.omega = tt_rank1('omega', [x, u, v], [ones, sinu, cosv])
        # self.omega = tt_rank1(
        ...
        self.u0 = tt_rank1('u0', self.indices, funcs)
...
if __name__ == "__main__":
    ...
    u0 = problem.u0.contract('o').value('o')
    plt.figure()
    plt.contourf(u0[0, :, :])
    plt.colorbar()
    plt.title('u0')
    
    omega = problem.omega.contract('o').value('o')
