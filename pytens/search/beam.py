...
class BeamSearch:
    ...
    def search(self, initial_state: SearchState, guided: bool = False):
        ...
        self.heap = [
            (-self.initial_cost, initial_state)
        ]  # the initial state has a very bad score
        ...
        if guided:
            ...
            with open("models/action.pkl", "rb") as action_model:
                trainer.op_picker.load_state_dict(
                    torch.load(action_model, weights_only=True)
                )

            with open("models/state.pkl", "rb") as state_model:
                trainer.state_to_torch.load_state_dict(
                    torch.load(state_model, weights_only=True)
                )
            ...
            for ac in state.get_legal_actions():
                ...
                for new_state in state.take_action(ac, params=self.params):
                    ...
                    self.stats["compression"].append(
                        (
                            time.time() - tic,
                            self.initial_cost / new_state.network.cost(),
                        )
                    )
