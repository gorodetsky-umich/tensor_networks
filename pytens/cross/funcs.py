...
class TensorFunc:
    ...
    def cost(self) -> int:
        """Get the size of the tensor function. The same as cost."""

        return int(np.prod(self.shape))

    def free_indices(self) -> List[Index]:
        """Get free indices of the function."""
...
class FuncNeutron(CountingFunc):
    ...
    def _check_job_status(self, job_id: str) -> bool:
        current_jobs = subprocess.check_output(
            "squeue -u zhgguo", stderr=subprocess.STDOUT, shell=True
        ).decode("utf-8")
    ...
    def _run(self, args: np.ndarray) -> np.ndarray:
        ...
        for i in range(args.shape[0]):
            ...
            job_result = (
                f"/home/zhgguo/neutron_diffusion/{self.d}/"
                + "_".join(str(x) for x in job)
                + ".txt"
            )
        ...
        with open(
            f"output/neutron_diffusion_{self.d}.pkl", "wb"
        ) as cache_file:
