...
def main_loop(config: Config, logger: logging.Logger, save_dir: pathlib.Path):
    ...
    fig_o = problem.plot_per_step_overlay(sol[0], disc)
    fig = problem.plot_per_step(sol[0], disc)
    ...
    for ii in (pbar := tqdm(range(1, config.solver.num_steps + 1))):
        ...
        if ii % config.saving.plot_freq == 0:
            ...
            fig_o = problem.plot_per_step_overlay(
                sol[ii],
                disc,
                fig=fig_o
            )
            ...
            if fig_o != None:
                fig_o.savefig(save_dir / (f'{config.problem}_overlay.pdf'))

            fig = problem.plot_per_step(sol[ii], disc)
            if fig != None:
                fig.savefig(
    ...
    fig = problem.plot_all_sols(sol, disc)
    if fig != None:
        fig.savefig(
